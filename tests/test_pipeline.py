...
<item><title>ジャングリア沖縄 夏休みの混雑状況とレンタカー需要が急増</title>
...
def test_rss2():
    ...
    items = parse_rss(RSS2_SAMPLE.format(recent=recent).encode(), "google_news", "沖縄 観光")
    assert len(items) == 2, items
    assert items[0]["title"].startswith("ジャングリア"), items[0]
...
def test_scoring(items):
    ...
    assert cands, "候補が0件"
    assert cands[0]["title"].startswith("ジャングリア"), cands
