...
```bash
cd okinawa-drive-blog
git init && git add -A && git commit -m "initial: blog automation system"
gh repo create okinawa-drive-blog --private --source=. --push
```
...
1. 独自ドメインを取得(例: okinawa-drive-lab.com)
2. 以下の3ファイルの `okinawa-drive-blog.example.com` を実ドメインに置換:
   - `astro.config.mjs`
...
※独自ドメインなしでも `<ユーザー名>.github.io/okinawa-drive-blog` で動作します(その場合も上記置換は必要)。
